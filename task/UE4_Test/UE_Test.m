...
%%% attempt to add hotkey to end trial early
...
ErrorITI = 0; %time in ms for ITI after error trial
TrialTime = 45000; % 45 Seconds Max duration
RewardMultiplier = [1 1 0]; %for High-Mid-Low 
...
if isempty(TrialRecord.LastTrialCodes.CodeNumbers) || ... if empty means it's the first trial
        (TrialRecord.TrialErrors(end) > 6) || ... %the subject was on black or did not complete the trial
        (TrialRecord.TrialErrors(end) < 7 && ...
        any(ismember(TrialRecord.LastTrialCodes.CodeNumbers, 58))) % Was the last trial South
    
    bIsNorth = 1;%trial direction 
    PreStartBehavCode = 57;
    CurrentGoals = {'Goal_NW', 'Goal_NE'};
else
    bIsNorth = 0;%trial direction    % comment this out if you want all trials to be north
...
wth1 = UE_WaitThenHold(goalTouch);
...
if ~wth1.Success
    ...
    %%%% send time-out trials to on_black BK
    tcp_.OnBlack();
    idle(ErrorITI);
    %%%%
    return
...
elseif TouchedValue < IgnoredValue 
    ...
    goodmonkey(400 * RewardMultiplier(TouchedValue), 'juiceline',1, 'numreward',3, 'eventmarker',50, 'nonblocking', 0); % 100 ms of juice x 2
%     tcp_.OnBlack();
