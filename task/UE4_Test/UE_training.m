...
elseif TouchedValue < IgnoredValue 
    ...
    goodmonkey(350 * RewardMultiplier(TouchedValue), 'juiceline',1, 'numreward',3, 'eventmarker',50, 'nonblocking', 0); % 100 ms of juice x 2
    tcp_.OnBlack();
elseif IgnoredValue < TouchedValue
    trialerror(6); % chose the wrong (second) object among the options [target distractor]
    goodmonkey(250 * RewardMultiplier(TouchedValue), 'juiceline',1, 'numreward',1, 'eventmarker',50, 'nonblocking', 0); % 100 ms of juice x 2
    tcp_.OnBlack();
    idle(ErrorITI);
end
